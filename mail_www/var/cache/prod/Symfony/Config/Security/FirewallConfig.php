...
require_once __DIR__.\DIRECTORY_SEPARATOR.'FirewallConfig'.\DIRECTORY_SEPARATOR.'MauticSsoConfig.php';
...
require_once __DIR__.\DIRECTORY_SEPARATOR.'FirewallConfig'.\DIRECTORY_SEPARATOR.'LightSamlSpConfig.php';
require_once __DIR__.\DIRECTORY_SEPARATOR.'FirewallConfig'.\DIRECTORY_SEPARATOR.'MauticPluginAuthConfig.php';
...
class FirewallConfig 
{
    ...
    public function mauticSso(array $value = []): \Symfony\Config\Security\FirewallConfig\MauticSsoConfig
    {
        if (null === $this->mauticSso) {
            $this->_usedProperties['mauticSso'] = true;
            $this->mauticSso = new \Symfony\Config\Security\FirewallConfig\MauticSsoConfig($value);
        } elseif (0 < \func_num_args()) {
            throw new InvalidConfigurationException('The node created by "mauticSso()" has already been initialized. You cannot pass values the second time you call mauticSso().');
        }

        return $this->mauticSso;
    }
    ...
    public function lightSamlSp(array $value = []): \Symfony\Config\Security\FirewallConfig\LightSamlSpConfig
    {
        if (null === $this->lightSamlSp) {
            $this->_usedProperties['lightSamlSp'] = true;
            $this->lightSamlSp = new \Symfony\Config\Security\FirewallConfig\LightSamlSpConfig($value);
        } elseif (0 < \func_num_args()) {
            throw new InvalidConfigurationException('The node created by "lightSamlSp()" has already been initialized. You cannot pass values the second time you call lightSamlSp().');
        }

        return $this->lightSamlSp;
    }

    public function mauticPluginAuth(array $value = []): \Symfony\Config\Security\FirewallConfig\MauticPluginAuthConfig
    {
        if (null === $this->mauticPluginAuth) {
            $this->_usedProperties['mauticPluginAuth'] = true;
            $this->mauticPluginAuth = new \Symfony\Config\Security\FirewallConfig\MauticPluginAuthConfig($value);
        } elseif (0 < \func_num_args()) {
            throw new InvalidConfigurationException('The node created by "mauticPluginAuth()" has already been initialized. You cannot pass values the second time you call mauticPluginAuth().');
        }

        return $this->mauticPluginAuth;
    }
    ...
    public function __construct(array $value = [])
    {
        ...
        if (array_key_exists('mautic_sso', $value)) {
            $this->_usedProperties['mauticSso'] = true;
            $this->mauticSso = new \Symfony\Config\Security\FirewallConfig\MauticSsoConfig($value['mautic_sso']);
            unset($value['mautic_sso']);
        }
        ...
        if (array_key_exists('light_saml_sp', $value)) {
            $this->_usedProperties['lightSamlSp'] = true;
            $this->lightSamlSp = new \Symfony\Config\Security\FirewallConfig\LightSamlSpConfig($value['light_saml_sp']);
            unset($value['light_saml_sp']);
        }

        if (array_key_exists('mautic_plugin_auth', $value)) {
            $this->_usedProperties['mauticPluginAuth'] = true;
            $this->mauticPluginAuth = new \Symfony\Config\Security\FirewallConfig\MauticPluginAuthConfig($value['mautic_plugin_auth']);
            unset($value['mautic_plugin_auth']);
        }
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['mauticSso'])) {
            $output['mautic_sso'] = $this->mauticSso->toArray();
        }
        ...
        if (isset($this->_usedProperties['lightSamlSp'])) {
            $output['light_saml_sp'] = $this->lightSamlSp->toArray();
        }
        if (isset($this->_usedProperties['mauticPluginAuth'])) {
            $output['mautic_plugin_auth'] = $this->mauticPluginAuth->toArray();
        }
